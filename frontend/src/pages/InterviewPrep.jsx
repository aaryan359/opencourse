...
const InterviewPrep = () => {
  ...
  const handleFieldSelect = async (field) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5001/interview/getallinterview/${encodeURIComponent(
          field
        )}`
      );
      ...
      if (response.data.length === 0) {
        setNoQuestionsMessage(
          `No questions available for ${field} at this moment.`
        );
        setQuestions([]);
      } else {
  ...
  const handleRating = (questionId, rating) => {
    setQuestions(
      questions.map((q) => (q._id === questionId ? { ...q, rating } : q))
    );
  };
      ...
      {questions.length > 0 && (
        <div className="mt-6">
          <h2 className="text-xl font-semibold">
            Questions in {selectedField}
          </h2>
          {questions.map((question, index) => (
            <div
              key={question._id}
              className="bg-gray-100 p-4 mb-4 rounded shadow hover:shadow-lg transition"
            >
              <p className="font-bold">
                {index + 1}. {question.text}{" "}
                <span className="italic">({question.company})</span>
              </p>
              <p>
                <strong>Answer:</strong>{" "}
                {question.showAnswer ? question.answer : "Hidden"}
              </p>
              <button
                onClick={() =>
                  setQuestions(
